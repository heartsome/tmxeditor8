...
public class TermConsistenceQA extends QARealization {
	...
	public TermConsistenceQA(){
		...
		tipLevel = preferenceStore.getInt(QAConstant.QA_PREF_term_TIPLEVEL);
	}
	...
	public String startQA(final QAModel model, IProgressMonitor monitor, IFile iFile, QAXmlHandler xmlHandler,
		...
		compareTerms(iFile, xmlHandler, tuTextMap);
	 ...
	 */
	private void compareTerms(IFile iFile, QAXmlHandler xmlHandler, Map<String, String> tuMap){
		String source_lan = tuMap.get("source_lan");
		String target_lan = tuMap.get("target_lan");
		
		boolean souTerm_Context = getContext(source_lan);
		boolean tarTerm_Context = getContext(target_lan);
		
		String sourceText = TextUtil.resetSpecialString(tuMap.get("srcPureText"));
		String targetText = TextUtil.resetSpecialString(tuMap.get("tarPureText"));
...
//		sourceText = TextUtil.cleanString(sourceText);
		...
		Vector<Hashtable<String, String>> termsVector = matcher.serachTransUnitTerms(sourceText, source_lan, target_lan, false);
		...
		for (int termIndex = 0; termIndex < termsVector.size(); termIndex++) {
			...
			if (containsTerm(sourceText, sourceTerm, souTerm_Context)) {
				if (!containsTerm(targetText, targetTerm, tarTerm_Context)) {
					...
					hasError = true;
					String qaType = Messages.getString("qa.all.qaItem.TermConsistenceQA");
					super.printQAResult(new QAResultBean(lineNumber, qaType, errorTip, iFileFullPath, langPair, rowId, tipLevel, QAConstant.QA_TERM));
				}
			}
		}
	}
