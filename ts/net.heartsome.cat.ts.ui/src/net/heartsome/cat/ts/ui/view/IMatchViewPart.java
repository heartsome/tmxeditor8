...
public interface IMatchViewPart {
	...
	/**
	 * 重新加载所有匹配
	 * @param editor 当前正在编辑的编辑器
	 * @param rowIndex 当前选中的文本段索引;
	 */
	void reLoadMatches(IXliffEditor editor, int rowIndex);
	
	/**
	 * 接受匹配面板中的第 index 个匹配
	 * @param index 匹配面板中的序号，从0开始
	 */
	 ...
	 */
	void manualExecComplexTranslation(IComplexMatch complexMatcher);
	
	/**
	 * 执行简单匹配（目前只实现了google,bing）
	 * @param simpleMatcher ;
	 */
	void manualExecSimpleTranslation(ISimpleMatcher simpleMatcher);
}
