...
public class ParagraphConsistenceQA extends QARealization {
	...
	public String startQA(QAModel model, IProgressMonitor monitor, IFile iFile, QAXmlHandler xmlHandler,
		...
		String xlfPath = tuMap.get("xlfPath");
		...
		if (model.isMuliFiles()) {
			compareNext(rowId, iFileFullPath, langPair);
		}else {
			compareNext(rowId, iFileFullPath, langPair);
		}
	 ...
	 * @param rowId
	 */
	private void compareNext(String rowId, String iFileFullPath, String langPair){
		// 先比较相同源文不同译文的情况
		String sameSourceqaType = Messages.getString("qa.ParagraphConsistenceQA.name1");
			...
			if (curList.contains(rowId)) {
				...
				for(String rowId_1 : curList){
					...
					if (sameSourceData.size() > 0) {
						...
						super.printQAResult(new QAResultBean("" + lineNum, sameSourceqaType, errorTip, iFileFullPath, langPair, rowId, tipLevel, QAConstant.QA_PARAGRAPH));
					}
				}
		...
		// 再比较相同译文不同源文的情况
		String sameTargetQaType = Messages.getString("qa.ParagraphConsistenceQA.name2");
			...
			if (curList.contains(rowId)) {
				...
				for(String rowId_1 : curList){
					...
					if (sameTargetData.size() > 0) {
						...
						super.printQAResult(new QAResultBean("" + lineNum, sameTargetQaType, errorTip, iFileFullPath, langPair, rowId, tipLevel, QAConstant.QA_PARAGRAPH));
					}
				}
