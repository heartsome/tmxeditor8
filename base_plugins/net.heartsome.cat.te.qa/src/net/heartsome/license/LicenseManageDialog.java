...
import net.heartsome.cat.ts.help.TeSystemResourceUtil;
...
public class LicenseManageDialog extends HsAbstractHelpDilog {
	...
	protected void createButtonsForButtonBar(Composite parent) {
		...
		if (type == Constants.STATE_NOT_ACTIVATED || type == Constants.STATE_INVALID || type == Constants.STATE_EXPIRED
				|| type == Constants.EXCEPTION_INT14 || type == Constants.EXCEPTION_INT15
				|| type == Constants.EXCEPTION_INT1 || type == Constants.EXCEPTION_INT2
				|| type == Constants.EXCEPTION_INT3 || type == Constants.EXCEPTION_INT4) {
			createButton(parent, IDialogConstants.OK_ID, Messages.getString("license.LicenseManageDialog.activeBtn"),
					true);
			isDefault = true;
		}
	...
	private void createStatusComp(Composite parent) {
		...
		if (type == Constants.STATE_NOT_ACTIVATED) {
			...
			btnCancelActive.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					boolean result = MessageDialog.openConfirm(getShell(),
							Messages.getString("license.LicenseManageDialog.confirm"),
						...
						try {
							...
							if (re == Constants.LOGOUT_SUCCESS) {
								MessageDialog.openInformation(getShell(),
										Messages.getString("license.LicenseManageDialog.notice"),
							...
							if (t instanceof java.security.cert.CertificateException) {
								MessageDialog.openInformation(getShell(), Messages
										.getString("license.LicenseManageDialog.titleNet"), MessageFormat.format(
										Messages.getString("license.LicenseManageDialog.infoNet"),
										Constants.EXCEPTION_STRING16));
							} else {
								MessageDialog.openInformation(getShell(), Messages
										.getString("license.LicenseManageDialog.titleNet"), MessageFormat.format(
										Messages.getString("license.LicenseManageDialog.infoNet"),
										Constants.EXCEPTION_STRING17));
							}
						}
					}
				}
			...
			typeLbl1.setText(Messages.getString(utilDate == null ? "license.LicenseManageDialog.typeBusiness"
					: "license.LicenseManageDialog.typeTemp"));
			...
			noticeLbl.setText(MessageFormat.format(Messages.getString("license.LicenseManageDialog.noticeLbl"),
			...
			GridData dataLabel = new GridData(GridData.FILL_HORIZONTAL);
			noticeLbl.setLayoutData(dataLabel);
			noticeLbl.setText(MessageFormat.format(
	...
	private void createActiveComp(Composite parent) {
		...
		link2.addMouseListener(new MouseListener() {
			...
			public void mouseDown(MouseEvent e) {
				Program.launch(Messages.getString("license.LicenseManageDialog.urlr8trial") + "&PRODUCT="
						+ ProtectionFactory.getProduct() + "&PLATFORM=" + ProtectionFactory.getPlatform());
			}
			...
			if (va.checkEdition()) {
				...
				try {
					...
					if (r == Constants.ACTIVE_OK_INT) {
						TeSystemResourceUtil.beforeload();
						TeSystemResourceUtil.load(false);
						setVisible(false);
						super.okPressed();
						MessageDialog.openInformation(getShell(),
								Messages.getString("license.LicenseManageDialog.notice"),
						...
						setVisible(false);
						MessageDialog.openInformation(getShell(), Messages
								.getString("license.LicenseManageDialog.notice"), MessageFormat.format(
								Messages.getString("license.LicenseManageDialog.activeFail"),
								StringUtils.getErrorCode(r)));
					}
				} catch (Exception e) {
					setVisible(false);
					e.printStackTrace();
					...
					if (t instanceof java.security.cert.CertificateException) {
						MessageDialog.openInformation(getShell(), Messages
								.getString("license.LicenseManageDialog.titleNet"),
								MessageFormat.format(Messages.getString("license.LicenseManageDialog.infoNet"),
										Constants.EXCEPTION_STRING16));
					} else {
						MessageDialog.openInformation(getShell(), Messages
								.getString("license.LicenseManageDialog.titleNet"),
				...
				// MessageDialog.openInformation(getShell(),
				// Messages.getString("license.LicenseManageDialog.titleInvalid"),
				// MessageFormat.format(Messages.getString("license.LicenseManageDialog.infoInvalid1"),
				// getVersionContent(System.getProperty("TSEdition")), getVersionContent(new
				// LicenseIdGenerator(getLicenseId()).getVersion())));
				...
				String message = MessageFormat.format(Messages.getString("license.LicenseManageDialog.infoInvalid1"),
						edition1, edition2);
				...
				list.add(map4);
				new CustomMessageDialog(getShell(), Messages.getString("license.LicenseManageDialog.titleInvalid"),
						message, list, editionInput).open();
			}
		} else {
	...
	private String getLicenseId() {
		return text1.getText() + text2.getText() + text3.getText() + text4.getText() + text5.getText()
				+ text6.getText();
	}
