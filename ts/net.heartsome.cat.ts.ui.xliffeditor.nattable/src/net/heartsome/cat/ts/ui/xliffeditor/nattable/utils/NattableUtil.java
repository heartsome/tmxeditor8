...
//				monitor.beginTask(Messages.getString("utils.NattableUtil.task1"), 8);
						...
						Display.getDefault().syncExec(new Runnable() {
							...
							public void run() {
								state[0] = MessageDialog.openConfirm(Display.getDefault().getActiveShell(),
										Messages.getString("utils.NattableUtil.msgTitle2"), Messages.getString("utils.NattableUtil.msg.cantConnDefaultDb"));
							}
						});
						if(!state[0]){
							return;
				...
				for (Entry<String, List<String>> entry : tmpGroup.entrySet()) {
					...
					if (importer.checkImporter()) {
						...
						StringBuffer fileContent = xliffEditor.getXLFHandler().generateTMXFileContent(systemUser, rowIdList,
								xliffEditor.getSrcColumnName(), xliffEditor.getTgtColumnName(),
