	...
	public void beginQA(final QAResult qaResult) {
		...
		Job job = new Job(Messages.getString("qa.all.qa")) {
			...
			protected IStatus run(IProgressMonitor monitor) {
				...
				//检查是否有文本段一致性检查，如果有，则先进行判断文本段是否完整。
				if (model.getBatchQAItemIdList().indexOf(QAConstant.QA_PARACOMPLETENESS) >= 0) {
					isParaComplete = true;
				}
				...
				for (int fileIndex = 0; fileIndex < model.getQaXlfList().size(); fileIndex++) {
					...
					for (int fileNodeIdx = QAConstant.QA_FIRST; fileNodeIdx <= fileNodeSum; fileNodeIdx++) {
						//获取语言对
						String source_lan = handler.getNodeAttribute(xlfPath, "/xliff/file[" + fileNodeIdx + "]", "source-language");
						String target_lan = handler.getNodeAttribute(xlfPath, "/xliff/file[" + fileNodeIdx + "]", "target-language");
						String langPair = source_lan + Hyphen + target_lan;
						...
						for (int tuIndex = QAConstant.QA_FIRST; tuIndex <= curFileTUNodeSum; tuIndex++) {
							...
							Map<String, String> tuMap = handler.getFilteredTUText(xlfPath, tuXpath, model.getNotInclude());
							
							//如果返回的map为null，则进行下一循环，行号也不加自加，这样可以保持这里的行号与界面上的行号一致性，方便定位
							if (tuMap == null) {
							...
							if (tuMap.size() == 0) {
								...
								continue;
							}else if ("".equals(tuMap.get("tarContent")) || tuMap.get("tarContent") == null ) {	//正常情况下应有四个值
								//因为文本段完整性检查要判断译文是否为空的情况，所以，如果译文为空，只有文本段完整性要进行检查，其他检查项都跳过。
								if (isParaComplete) {
									tuMap.put("lineNumber", "" + lineNumber);
									tuMap.put("iFileFullPath", iFileFullPath);
									tuMap.put("langPair", langPair);
									getClassInstance(QAConstant.QA_PARACOMPLETENESS).startQA(model, subMonitor, iFile, handler,  tuMap);
								}
								if (!monitorWork(subMonitor, traversalTuIndex, false)) {
									closeDB();
									return Status.CANCEL_STATUS;
								}
								qaResult.sendDataToViewer(tuMap.get("rowId"));
								continue;
							}
							...
							tuMap.put("iFileFullPath", iFileFullPath);
							tuMap.put("langPair", langPair);
							tuMap.put("xlfPath", xlfPath);
							tuMap.put("source_lan", source_lan);
							tuMap.put("target_lan", target_lan);
							
							tuMap.put("fileNodeIndex", "" + fileNodeIdx);
							tuMap.put("tuNodeIndex", "" + tuIndex);
							...
							for (int i = 0; i < model.getBatchQAItemIdList().size(); i++) {
								...
								// 开始进行该项文件的该项检查
								final String result = realization.startQA(model, subMonitor, iFile, handler, tuMap);
				...
				for(Entry<String, ArrayList<String>> langEntry : languageList.entrySet()){
					...
					for(String tgtLang : langEntry.getValue()){
						...
						for(String rowId : rowIdsList){
							...
							Map<String, String> tuMap = handler.getFilteredTUText(filePath,
							...
							if (tuMap.size() == 0) {
								...
								if (isParaComplete) {
									tuMap.put("lineNumber", "" + lineNumber);
									tuMap.put("iFileFullPath", iFile.getFullPath().toOSString());
									tuMap.put("langPair", langPair);
									getClassInstance(QAConstant.QA_PARACOMPLETENESS).startQA(model, subMonitor, iFile, handler, tuMap);
								}
								if (!monitorWork(subMonitor, traversalTuIndex, false)) {
							...
							tuMap.put("iFileFullPath", iFile.getFullPath().toOSString());
							tuMap.put("langPair", langPair);
							tuMap.put("source_lan", srcLang);
							tuMap.put("target_lan", tgtLang);
