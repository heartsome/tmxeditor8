				...
				for (File slideMasterFile : arrSlideMasterFile) {
					...
					if (slideMasterFile.isFile() && slideMasterFile.getName().toLowerCase().endsWith(".xml")) {
						if (vg.parseFile(slideMasterFile.getAbsolutePath(), true)) {
							...
							while (ap.evalXPath() != -1) {
								String strType = vu.getElementAttribute(".//p:ph", "type");
								String idx = vu.getElementAttribute(".//p:ph", "idx");
								if (strType == null && idx == null) {
									continue;
								}

								String strX = vu.getElementAttribute(".//a:xfrm/a:off", "x");
								String strY = vu.getElementAttribute(".//a:xfrm/a:off", "y");
			...
			for (final String slideName : lstSlide) {
				...
				if (vg.parseFile(strTmpFolderPath + File.separator + "ppt" + File.separator + "slides" + File.separator
					...
					while (apRels.evalXPath() != -1) {
						String strTarget = vu.getCurrentElementAttribut("Target", null);
						if (strTarget != null) {
				...
				if (vg.parseFile(strSlideFilePath, true)) {
					...
					while (apSlide.evalXPath() != -1) {
						String strX = vu.getElementAttribute(".//p:xfrm/a:off | .//a:xfrm/a:off", "x");
						String strY = vu.getElementAttribute(".//p:xfrm/a:off | .//a:xfrm/a:off", "y");
						String strType = vu.getElementAttribute(".//p:ph", "type");
						String strIdx = vu.getElementAttribute(".//p:ph", "idx");
						...
						if (name.equals("p:sp")) {
							...
							if (apParent.evalXPath() != -1) {
								parentX = vu.getElementAttribute("./p:grpSpPr/a:xfrm/a:off", "x");
								parentY = vu.getElementAttribute("./p:grpSpPr/a:xfrm/a:off", "y");
							}
						}
						vn.pop();
						if (strX == null && strY == null) {
