...
public class MatchViewPart extends ViewPart implements ISelectionListener, IMatchViewPart {
	...
	private TransUnitBean currentTransUnitBean;
	...
	public void selectionChanged(final IWorkbenchPart part, final ISelection selection) {
		...
		if (object instanceof Integer) {
			...
			currentProject = input.getFile().getProject();
			currentTransUnitBean = transUnit;
			executeMatch(rowId, transUnit, currentProject);
		}
	}

	private void executeMatch(String rowId, TransUnitBean transUnit, IProject project) {
		if (matcherThread == null) {
		...
		synchronized (matchDataContainer) {
			...
			matchDataContainer.add(new MatchData(transUnit, project, rowId));
	...
	public void reLoadMatches(IXliffEditor editor, int rowIndex) {
		...
		executeMatch(handler.getRowId(rowIndex), currentTransUnitBean, currentProject);
	}
	...
	public void manualExecComplexTranslation(IComplexMatch complexMatcher) {
		if (rowIndex != -1 && currentProject != null && currentTransUnitBean != null) {
			synchronized (manualTranslationTaskContainer) {
				manualTranslationTaskContainer.pushTranslationTask(complexMatcher);
				manualTranslationTaskContainer.notify();
			}
		}
	}

	public void manualExecSimpleTranslation(ISimpleMatcher simpleMatcher) {
		if (rowIndex != -1 && tuInfoBean != null) {
			synchronized (manualTranslationTaskContainer) {
				manualTranslationTaskContainer.pushTranslationTask(simpleMatcher);
				manualTranslationTaskContainer.notify();
			}
		}
	}
	...
	class ExecuteMatchThread extends Thread {
		 ...
		 *            ;
		 */
		private void updateUI(Vector<AltTransBean> tmAltTrans) {
			...
			if (itemSize > 0) {
				...
				if ((tgt == null || tgt.equals("")) && transParameter.isApplyTmMatch() && tmAltTrans.size() > 0) {
					AltTransBean aBean = tmAltTrans.get(0);
					String tgtContent = aBean.getTgtContent();
					if (transUnit != null) {
						String srcContent = transUnit.getSrcContent();
						if (srcContent != null && !srcContent.equals("") && transParameter.isAdjustSpacePosition()) {
							tgtContent = TmUtils.adjustSpace(srcContent, tgtContent);
						}
					}
					String matchType = aBean.getMatchProps().get("hs:matchType");
					String quality = aBean.getMatchProps().get("match-quality").trim();
					if (!matchType.equals("TM")) {
						matchType = null;
						quality = null;
					}
					editor.affterFuzzyMatchApplayTarget(rowIndex, tgtContent, matchType, quality);
				}
	...
	class ManualTranslationTread extends Thread {
		...
		public void run() {
			while (!stop) {
				...
				Object obj = container.popTranslationTask();
				if (obj == null) {
					interrupt = false;
					continue;
				}
				if (obj instanceof ISimpleMatcher) {
					if (rowIndex == -1 || tuInfoBean == null) {
						continue;
					}
					if (interrupt) {
					...
					final ISimpleMatcher simpleMatcher = (ISimpleMatcher) obj;
				...
				} else if (obj instanceof IComplexMatch) {
					IComplexMatch complexMatch = (IComplexMatch) obj;
					...
					if (rowIndex != -1 && currentProject != null && currentTransUnitBean != null) {
						...
						Vector<AltTransBean> result = complexMatch.executeTranslation(currentTransUnitBean,
								currentProject);
						if (interrupt) {
							interrupt = false; // reset for next task
							updateStatusInfo(null);
	...
	private synchronized void loadData(List<AltTransBean> matches) {
		...
		for (AltTransBean altTransBean : matches) {
			...
			StringBuffer sbSrc = new StringBuffer(srcContent);
			InnerTagUtil.parseXmlToDisplayValue(sbSrc, TagStyle.getDefault(false));
			gridItem.setText(0, srcContent/* InnerTagUtil.resolveTag(sbSrc.toString()) */);

			gridItem.setText(1, quality);

			StringBuffer sbTgt = new StringBuffer(tgtContent);
			InnerTagUtil.parseXmlToDisplayValue(sbTgt, TagStyle.getDefault(false));
			gridItem.setText(2, tgtContent/* InnerTagUtil.resolveTag(sbTgt.toString()) */);
