...
public class DatabaseService {
	 ...
	 * @return 导入结果，为int型数据，参考{@link ImportAbstract}中的定义;;
	 */
	public static int importTbxWithString(String tbxStr, IProgressMonitor monitor, MetaData metaData, int strategy,
			String srcLang) {
		if (tbxStr != null && tbxStr.length() > 0) {
			DBOperator dbOp = getDBOperator(metaData);
			try {
				dbOp.start();
				...
				LOGGER.error(Messages.getString("service.DatabaseService.logger7")
						+ dbOp.getMetaData().getDatabaseName(), e);
				return ImportAbstract.FAILURE_3;
			} catch (ClassNotFoundException e) {
				LOGGER.error(Messages.getString("service.DatabaseService.logger8"), e);
				return ImportAbstract.FAILURE_3;
			} finally {
				if (dbOp != null) {
					try {
						...
						LOGGER.error(Messages.getString("service.DatabaseService.logger9"), e);
					}
				}
			}
		}
	...
	public static int importTbxWithFile(String fileName, IProgressMonitor monitor, MetaData metaData, int strategy)
		...
		if (file.exists() || !file.isDirectory()) {
			...
			try {
				dbOp.start();
				ImportAbstract impOp = new ImportTbx(dbOp, strategy);
				int result = impOp.doImport(fileName, monitor);
				return result;
			} catch (SQLException e) {
