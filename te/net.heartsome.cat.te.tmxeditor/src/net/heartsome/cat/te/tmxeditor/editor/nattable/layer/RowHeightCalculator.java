 ...
 */
package net.heartsome.cat.te.tmxeditor.editor.nattable.layer;
...
public class RowHeightCalculator {
	...
	public RowHeightCalculator(BodyLayer bodyLayer, NatTable table, int defaultRowHeight) {
	...
	public int recaculateRowHeight(int rowIndex) {
		int height = bodyLayer.getDataLayer().getRowHeightByPosition(rowIndex);
		// if (height == defaultRowheight) {
		height = getPreferredRowHeight(rowIndex, table.getConfigRegistry(), table.getClientArea().height);
		bodyLayer.getDataLayer().setRowHeightByPositionWithoutEvent(rowIndex, height);
		// }
		return height;
	}
