...
//				copyFile(skeletonFile, "/home/robert/Desktop/a.txt");
				...
				//先处理ttx文件
				/* 注释，只支持预翻译后的文件。
				 * ArrangeTTX arrange = new ArrangeTTX(sklVN, sklXM, elementSegmentation, 
						initSegmenter, userSourceLang, catalogue, detectedSourceLang, detectedTargetLang);
				arrange.createTuNode(skeletonFile);*/
...
//				copyFile(skeletonFile, "/home/robert/Desktop/aSkeletonFile.txt");
//				copyFile(xliffFile, "/home/robert/Desktop/aXliff.txt");
				...
				ConverterUtils.throwConverterException(Activator.PLUGIN_ID, MessageFormat.format(Messages.getString("ttx.Ttx2Xliff.warn"), new Object[]{new File(inputFile).getName()}));
			} catch (Exception e) {
				e.printStackTrace();
				ConverterUtils.throwConverterException(Activator.PLUGIN_ID, Messages.getString("ttx.Ttx2Xliff.msg1"), e);
			} finally {
				monitor.done();
				try {
					output.close();
		...
		private void check() throws MissTranslationException {
			sklVN.push();
			try {
				...
				while (ap.evalXPath() != -1) {
					String text = sklVN.toString(sklVN.getCurrentIndex()).replaceAll("\\s", "");
					if (text.length() > 0) {
		...
		private void parseSkeletonFile() throws Exception {
			...
			if (vg.parseFile(skeletonFile, true)) {
				...
				errorInfo = MessageFormat.format("无法解析源文件 {0} 。", 
		...
		private void analyzeNodes() throws Exception {
			...
			while (ap.evalXPath() != -1) {
				...
				sklVN.pop();
				//<Tuv Lang="EN-US">See the enclosure <ut DisplayText="xr">&lt;:xr &quot;“Specifications” on page&lt;:hs&gt;20&quot; 2&gt;</ut>.</Tuv>
		...
		/**
		 * 分析处理源或目标文本，主要是处理cf标记，若第一行的标记为一个</cf>，那么删除，
		 * 若缺少<cf>的结束标记，自动补全，还有种情况就是<cf>标记在tu节点之外的情况。
		 */
...
//			}
		...
		/**
		 * 分析处理目标文本，主要是处理cf标记，若第一行的标记为一个</cf>，那么以ph的形式出现，
		 * 若缺少<cf>的结束标记，自动补全，还有种情况就是<cf>标记在tu节点之外的情况。
		 ...
		 * @throws IOException
		 */
		private void writeSegment(String source, String target, int match,
				boolean isLock, boolean isXtranslate) throws IOException {
			String approved = match > 100 ? "yes" : "no"; 
			String matchStr = match > 0 ? " hs:matchType=\"TM\" hs:quality=\"" + match + "\"" : "";
			if (!isLock) {
				writeString("  <trans-unit id=\"" + segId 
						+ "\" xml:space=\"preserve\" approved=\"" + approved + "\" addedEndTagNum='"+addedEndTagNum+"'>\n" 
						+ "    <source xml:lang=\"" + userSourceLang + "\">"); 
			} else {
				writeString("  <trans-unit id=\"" + segId 
						+ "\" xml:space=\"preserve\" translate=\"no\" approved=\"" + approved + "\" addedEndTagNum='"+addedEndTagNum+"'>\n" 
						+ "    <source xml:lang=\"" + userSourceLang + "\">"); 
			}
			writeString(source + "</source>\n "); 
			if (!target.equals("")) { 
				if (!targetLang.equals("")) { 
					writeString("    <target xml:lang=\"" + targetLang + "\" state=\"new\""+ matchStr +">" + target + "</target>\n"); 
				} else {
