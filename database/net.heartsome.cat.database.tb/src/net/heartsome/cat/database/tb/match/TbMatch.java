...
public class TbMatch extends AbstractTbMatch implements ITbMatch {
	...
	public Vector<Hashtable<String, String>> getTransUnitTerms() {
		...
		for (int i = 0; i < dbOperatorList.size(); i++) {
			try {	
				// 	如果为sqlite数据库，将文本转换成大写查找
				if(("SQLite").equals(dbOperatorList.get(i).getMetaData().getDbType())){
					temp =super.srcPureText.toUpperCase();
				}else{
					temp = super.srcPureText;
				}
				terms.addAll(dbOperatorList.get(i).findAllTermsByText(temp, super.srcLanguage,
						super.tgtLanguage));
			} catch (Exception e) {
				e.printStackTrace();
			}
