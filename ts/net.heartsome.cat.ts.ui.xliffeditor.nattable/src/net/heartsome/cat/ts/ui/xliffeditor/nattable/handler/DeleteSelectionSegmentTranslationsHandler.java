			...
			try {
				...
				for (int i = 0; i < selectedRows.length; i++) {
					...
					if (null != tgt) {
						if (tgt.equals("no")) {
							exitFlag = true;
							message.append(handler.getRowIndex(handler.getRowId(selectedRows[i])) + 1 + ",");
							continue;
						}
					}
				...
				if (exitFlag) {
					...
					MessageDialog.openInformation(xliffEditor.getSite().getShell(), Messages
							.getString("handler.DeleteSelectionSegmentTranslationsHandler.msgTitle2"), MessageFormat
							.format(Messages.getString("handler.DeleteSelectionSegmentTranslationsHandler.msg2"),
									message.toString()));
				}
				if (rows.size() != 0) {
