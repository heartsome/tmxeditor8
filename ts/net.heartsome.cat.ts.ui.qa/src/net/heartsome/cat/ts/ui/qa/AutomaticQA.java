...
public class AutomaticQA implements IAutomaticQA{
	...
	public void autoQA(String rowId, StringBuffer resultSB, boolean isAddToDb){
		...
		Map<String, String> tuMap = handler.getAutoQAFilteredTUText(rowId, filterMap);
		if (tuMap.size() == 0){
			return;
		}
		...
		String langPair = source_lan + "->" + target_lan;
		tuMap.put("langPair", langPair);
		String lineNumber = "" + (handler.getRowIndex(rowId) + 1);
		tuMap.put("lineNumber", lineNumber);
		tuMap.put("rowId", rowId);
		
		if ("".equals(tuMap.get("tarContent")) || tuMap.get("tarContent") == null ) {	//正常情况下应有四个值
			return;
		}
		...
		for (int i = 0; i < model.getBatchQAItemIdList().size(); i++) {
			...
			// 开始进行该项文件的该项检查
			String result = realization.startQA(model, monitor, iFile, xmlHandler, tuMap);
