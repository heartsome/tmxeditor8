...
public class TextUtil {
	...
	/**
	 * 获取　UUID ，唯一字符串。可用于主键或者　xml 文件的　id.	robert	2013-06-19
	 * @return
	 */
	...
	/**
	 * 清除所有空格，包括中文的，还有英文的、还有制表符	--robert	2013-12-10
	 */
	public static String trimSpace(String text){
		...
		while ((st < len) && (val[off + st] == ' ' || val[off + st] == '　' || val[off + st] == '\t')) {	
		    st++;
		}
		while ((st < len) && (val[off + len - 1] == ' ' || val[off + len - 1] == '　' || val[off + len - 1] == '\t')) {
		    len--;
		}
	...
	/**
	 * 判断指定字符串是否为空	--robert	2013-12-10
	 * @param text
	 * @return ;
	 */
	public static boolean checkStringEmpty(String text){
		return text == null || TextUtil.trimString(text).isEmpty();
	}
	
	/**
	 * 清除文本段中的换行符	--robert	2013-12-11
	 * @param text
	 * @return ;
	 */
	...
	/**
	 * 清理字符串，包括清除段首段末的空白字符，以及段中的换行符	--robert	2013-12-11
	 * @param text
	 * @return ;
	 */
	...
	public static String attributeValue(String value) {
		...
		do  {
			...
			if (value.charAt(offset) == '"') {
				list.add(new StringBuilder().append("'").append(value.substring(offset, offset + len)).append("'").toString());
			} else {
				list.add(new StringBuilder().append('"').append(value.substring(offset, offset + len)).append('"').toString());
			}
			
		} while ((l = groupString(value, (int) l + (int) (l >> 32))) != -1);
	...
	// length, offset
	private static long groupString(String str, int index) {
		if (index >= str.length()) {
			return -1;
		}
