...
public class TagConsistenceQA extends QARealization{
	...
	public TagConsistenceQA(){
		...
		tipLevel = preferenceStore.getInt(QAConstant.QA_PREF_tag_TIPLEVEL);
	}
	...
	public String startQA(QAModel model, IProgressMonitor monitor, IFile iFile, QAXmlHandler xmlHandler,
		...
		if (!(srcContent.indexOf("<") >=0 || srcContent.indexOf("</") >=0 || srcContent.indexOf("/>") >= 0) 
				&& !(tarContent.indexOf("<") >=0 || tarContent.indexOf("</") >=0 || tarContent.indexOf("/>") >= 0)) {
			return "";
		}
		
		srcContent = "<xml>" + srcContent + "</xml>";
		tarContent = "<xml>" + tuMap.get("tarContent") + "</xml>";
		
		List<Map<String, String>> sourceTagList = xmlHandler.getTUTag(srcContent);
		List<Map<String, String>> targetTagList = xmlHandler.getTUTag(tarContent);
		...
		if ((loseTagSize = loseTag.size()) > 0) {
			String qaType = Messages.getString("qa.TagConsistenceQA.name1");
			String tagLoseTip = "";
			...
			hasError = true;
			super.printQAResult(new QAResultBean(lineNumber, qaType, errorTip, iFileFullPath, langPair, rowId, tipLevel, QAConstant.QA_TAG));
		}
		
		//处理标记不一致的情况
		int changeTagSize;
		if ((changeTagSize = changeTag.size()) > 0) {
			String qaType = Messages.getString("qa.TagConsistenceQA.name2");
			String tagChangeTip = "";
		...
		if ((moreTagSize = targetTagList.size()) > 0) {
			String qaType = Messages.getString("qa.TagConsistenceQA.addName1");
			String moreTagTip = "";
