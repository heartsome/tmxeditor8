...
public class TeActiveCellEditor {
	...
	public static void activate(ICellEditor cellEditor, Composite parent, Object originalCanonicalValue, Character initialEditValue, IDisplayConverter displayConverter, IStyle cellStyle, IDataValidator dataValidator, ICellEditHandler editHandler, int columnPosition, int rowPosition, int columnIndex, int rowIndex) {
		...
		TeActiveCellEditor.cellEditor = cellEditor;
		TeActiveCellEditor.editHandler = editHandler;
		TeActiveCellEditor.dataValidator = dataValidator;
		TeActiveCellEditor.columnPosition = columnPosition;
		TeActiveCellEditor.rowPosition = rowPosition;
		TeActiveCellEditor.columnIndex = columnIndex;
		TeActiveCellEditor.rowIndex = rowIndex;
	...
	public static void commitWithoutClose(){
		if (isValid() && validateCanonicalValue()) {
			editHandler.commit(MoveDirectionEnum.NONE, false);
		}
	}
	...
	public static void recalculateCellsBounds(){
		if(TeActiveCellEditor.isValid()){
			...
			NatTable table = (NatTable) activeCellEditorControl.getParent();
			HorizontalViewportLayer vLayer = LayerUtil.getLayer(table, HorizontalViewportLayer.class);
			int rowPosition = vLayer.getRowPositionByIndex(TeActiveCellEditor.getRowIndex());
			int columnPosition = vLayer.getColumnPositionByIndex(TeActiveCellEditor.getColumnIndex());
			rowPosition += 1;
			if (rowPosition < 1) {
				return;
			}			
			...
			if (cellBounds != null) {
				Rectangle adjustedCellBounds = table.getLayerPainter().adjustCellBounds(cellBounds);
				TeActiveCellEditor.getControl().setBounds(adjustedCellBounds);
				TeActiveCellEditor.columnPosition = columnPosition;
				TeActiveCellEditor.rowPosition = rowPosition;
			}
		}
