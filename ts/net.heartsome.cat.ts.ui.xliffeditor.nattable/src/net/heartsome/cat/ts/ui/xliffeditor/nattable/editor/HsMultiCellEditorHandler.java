...
public class HsMultiCellEditorHandler implements ICellEditHandler {
	...
	public HsMultiCellEditorHandler(StyledTextCellEditor cellEditor, ILayer layer, int columnPosition, int rowPosition) {
	...
	public boolean commit(MoveDirectionEnum direction, boolean closeEditorAfterCommit) {
		...
		if (cellEditor.isEditable()) {
			...
			layer.doCommand(new UpdateDataCommand(layer, columnPosition, rowPosition, canonicalValue));
		}		
		return true;
	}
