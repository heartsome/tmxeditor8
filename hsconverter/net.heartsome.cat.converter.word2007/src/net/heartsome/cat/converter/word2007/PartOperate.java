...
//	   </w:hyperlink>
	...
	protected void analysisNodeP() throws Exception {
		...
		if (textCount == 1) {
			...
			childAP.selectXPath("./node()[(name()='w:r' or name()='w:hyperlink') and not(@w:anchor) and not(descendant::node()[name()='w:p']) and descendant::w:t/text()!='']");
			while(childAP.evalXPath() != -1){
				segSB.append(getText());
			}
			if (deleteBlank(segSB.toString()).length() <= 0) {
				vn.pop();
				return;
			}
			
//			System.out.println("segSB.toString() =" + segSB.toString());
//			if (segSB.toString().indexOf("Heartsome 技术支持服务不收取任何费用。") != -1) {
